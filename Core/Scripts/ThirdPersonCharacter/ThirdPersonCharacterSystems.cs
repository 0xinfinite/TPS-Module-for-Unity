    ...
    public partial  struct ThirdPersonCharacterPhysicsUpdateSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        //foreach(var (aspect, rootMotion, entity) in SystemAPI.Query<ThirdPersonCharacterAspect, RootMotionComponent>().WithAll<ThirdPersonCharacterAspect,RootMotionComponent>()
        //    .WithEntityAccess().WithAll<Simulate>())
        //{
        //    //UnityEngine.Debug.Log("It has rootmotion: " + entity.Index);
        //    aspect.PhysicsUpdate(ref _context, ref _baseContext, rootMotion);
        //}
        ThirdPersonCharacterRootMotionPhysicsUpdateJob rootMotionJob = new ThirdPersonCharacterRootMotionPhysicsUpdateJob
        {
            Context = _context,
        ...
        rootMotionJob.Schedule(); //ScheduleParallel();

        ThirdPersonCharacterPhysicsUpdateJob job = new ThirdPersonCharacterPhysicsUpdateJob
        {
            Context = _context,
            BaseContext = _baseContext,
            //RootMotionLookup = SystemAPI.GetComponentLookup<RootMotionComponent>(true),
    ...
    public partial  struct ThirdPersonCharacterRootMotionPhysicsUpdateJob : IJobEntity, IJobEntityChunkBeginEnd
    {
        ...
        void Execute(ThirdPersonCharacterAspect characterAspect, RootMotionComponent rootMotion, Entity entity)
        {
            characterAspect.PhysicsUpdate(ref Context, ref BaseContext, rootMotion);
            ecb.RemoveComponent<RootMotionComponent>(entity);
        }
